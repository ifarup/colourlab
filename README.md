...
[![Build Status](https://travis-ci.org/ifarup/colourspace.svg?branch=master)](https://travis-ci.org/ifarup/colourspace)
[![Code Coverage](https://codecov.io/gh/ifarup/colourspace/branch/master/graph/badge.svg)](https://codecov.io/gh/ifarup/colourspace)
[![Documentation Status](https://readthedocs.org/projects/colourspace/badge/?version=latest)](http://colourspace.readthedocs.io/en/latest/?badge=latest)
...
Colour data, i.e., colour points, lists of colours, colour images etc., are most conveniently represented by objects of the colourspace.data.Data class. In order to construct such an object, the colour data and the colour space has to be specified. Typically, if `col_xyz` is an ndarray with colour data in XYZ, a colour data object can be constructed as

```python
col_data = colourspace.data.Data(colourspace.space.xyz, col_xyz)
```

Then the corresponding ndarray data in other colour spaces can be acquired by the get method:

```python
col_srgb = col_data.get(colourspace.space.srgb)
col_lab  = col_data.get(colourspace.space.cielab)
col_xyY  = col_data.get(colourspace.space.xyY)
```
...
* **colourspace.space.xyz**: The CIE XYZ colour space.
* **colourspace.space.xyY**: The CIE xyY colour space.
* **colourspace.space.cielab**: The CIELAB colour space with D65 white point.
* **colourspace.space.cielch**: Polar coordinates in the CIELAB colour space.
* **colourspace.space.cieluv**: The CIELUV colour space with D65 white point.
* **colourspace.space.ciecat02**: The colour space of the CIECAT02 colour adaptation transform.
* **colourspace.space.ciede00lab**: The underlying colourspace of the CIEDE2000 colour metric.
* **colourspace.space.srgb**: The sRGB colour space.
* **colourspace.space.rgb_adobe**: The Adobe RGB colour space.
* **colourspace.space.ipt**: The IPT colour space.
* **colourspace.space.lgj_osa**: The OSA-UCS colour space.
* **colourspace.space.lgj_e**: The Euclidised OSA-UCS colour space used in the &Delta;E<sub>E</sub> metric.
* **colourspace.space.din99x**: The various DIN99x colour spaces for the corresponding metrics (x is empty, b, c, or d).

There are also built-in colour data sets available. They are all represented by Data objects that can be constructed upon need by functions in the colourspace.data module. These functions have names starting with `d_`. Most of these data sets are mainly of interest for colour metrics researcher, but some of them will have broader interest, such as the various CIE colour matching functions, and the data of the Munsell patches.

Computing Colour Metrics
------------------------

The most common colour metrics are available as functions in the colourspace.metrics module. All the metric funtions take two colour Data objects as parameters. The two objects must be of the same dimension. If the colour data in the Data objects are of the dimension Nx...xMx3, the return value of the metric functions are ndarrays of dimension Nx...XM. For example, the &Delta;E<sub>ab</sub> colour difference between the two datasets `dataset1` and `dataset2` is computed as

```python
diff = colourspace.metric.dE_ab(dataset1, dataset2)
```

The following metrics are available:

* **colourspace.metric.dE_ab**: The CIE76 standard &Delta;E<sub>ab</sub> &ndash; the Euclidean distance in CIELAB.
* **colourspace.metric.dE_uv**: The Euclidean metric in CIELUV, &Delta;E<sub>uv</sub>.
* **colourspace.metric.dE_00**: The CIEDE2000 non-Euclidean colour metric.
* **colourspace.metric.dE_E**: The Euclidean colour metric &Delta;E<sub>E</sub>.
* **colourspace.metric.dE_DIN99x**: The DIN99x colour metrics (where x is empty, b, c or d).

Additionally, a general Euclidean colour metric in a given colour space can be computed as

```python
my_diff = colourspace.metric.euclidean(my_colour_space, dataset1, dataset2)
```
...
```python
my_rgb_linear = colourspace.space.TransformLinear(colourspace.space.xyz, my_M)
my_rgb = colourspace.space.TransformGamma(my_rgb_linear, my_gamma)
```
...
* **colourspace.space.TransformLinear**: A linear transformation defined by a 3x3 matrix (represented as a ndarray of shape (3,3)).
* **colourspace.space.TransformGamma**: A gamma correction applied individually to all channels. If the channel values are negative (should not be the case for RGB type spaces, but occurs, e.g., in IPT), the gamma transform is applied to the absolute value of the channel, and the sign is added in the end.
...
* **colourspace.space.TransformCartesian**: The oposite of the above.
* **colourspace.space.TransformxyY**: The projective transform from, e.g., XYZ to xyY, with that order of the coordinates.
...
* **colourspace.space.TransformCIELUV**: The non-linear transformation from XYZ to CIELUV, including the linear part for small XYZ values.The white point is a parameter, so this transformation can be used also to create CIELUV D50, etc.
* **colourspace.space.TransformSRGB**: The non-linear gamma-like correction from linear RGB with sRGB primaries to the non-linear sRGB colour space.

In addition, the following more special colour space transforms are available:

* colourspace.space.TransformLogCompressL
* colourspace.space.TransformLogCompressC
* colourspace.space.TransformPoincareDisk
* colourspace.space.TransformCIEDE00
* colourspace.space.TransformLGJOSA
* colourspace.space.TransformLGJE

Adding to this, new colour space transforms can be defined as classes inheriting from the colourspace.space.Transform base class.

Common white points are available as the following Data objects:

* colourspace.space.white_A
* colourspace.space.white_B
* colourspace.space.white_C
* colourspace.space.white_D50
* colourspace.space.white_D55
* colourspace.space.white_D65
* colourspace.space.white_D75
* colourspace.space.white_E
* colourspace.space.white_F2
* colourspace.space.white_F7
* colourspace.space.white_F11
...
### Constructing Gamut 
To construct a new Gamut we need to provide a colour space in the format provided by colourspace.space, 
and data/colour points in the format given the colourspace.data.Data class. If we want to construct the new Gamut in the colourspace RGB and the fictive points my_points, we would do the following:
...
## Examples
For all examples:
* **space:** a colourspace.space.Space object
* **c_data:** a colourspace.data.Data object
...
#### HPminDE()
The function receives one parameter. The points in the c_data format, Maps all points that lie outside of the gamut to the nearest point on the plane formed by the point and the L axe in the CIELAB colour space. Returns coordinate for the closest point on plane in the colourspace.data.Data format.
```
...
#### minDE()
The function receives one parameter. The points in the c_data format, and maps all points that lie outside of the gamut to the nearest point on the gamut in CIELAB colour space. Returns the nearest point in the colourspace.data.Data format.
```
...
`compress_axis(sp, c_data, ax):`              | Compresses the points linearly in the desired axel and colour space.          | colourspace.data.Data object
`HPminDE(c_data):`                            | Get coordinate for the closest point on plane and return mapped points        | colourspace.data.Data object
`minDE(c_data):`                              | Get nearest point and return mapped points.                                   | colourspace.data.Data object

test_colour
===========
This is a test package containing one test module for each module in colourspace. The test modules does unittesting 
